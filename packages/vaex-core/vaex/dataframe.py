...
class DataFrame(object):
    ...
    def dtype(self, expression, array_type=None, internal=False):
        """Alias to df.data_type"""
        return self.data_type(expression, array_type=array_type)
    ...
    def to_astropy_table(self, column_names=None, selection=None, strings=True, virtual=True, index=None, parallel=True):
        ...
        for name, data in self.to_items(column_names=column_names, selection=selection, strings=strings, virtual=virtual, parallel=parallel):
            if vaex.array_types.is_string_type(self.dtype(name)):  # for astropy we convert it to unicode, it seems to ignore object type
    ...
    def describe(self, strings=True, virtual=True, selection=None):
        ...
        for feature in self.get_column_names(strings=strings, virtual=virtual)[:]:
            ...
            if not isinstance(data_type, np.dtype):
                if self.dtype(feature) in array_types.string_types:
    ...
    def get_column_names(self, virtual=True, strings=True, hidden=False, regex=None, alias=True):
        ...
        def column_filter(name):
            ...
            if not virtual and name in self.virtual_columns:
                return False
            if not strings and vaex.array_types.is_string_type(self.dtype(name)):
                return False
    ...
    def _evaluate_implementation(self, expression, i1=None, i2=None, out=None, selection=None, filtered=True, array_type=None, parallel=True, chunk_size=None, raw=False):
        ...
        if parallel:
            ...
            for expression in set(expressions):
                dtypes[expression] = dtype = df.dtype(expression)
                ...
                if array_types.is_string_type(dtype) or use_filter or selection or not isinstance(dtype, np.dtype):
            ...
            if column_name not in self.get_column_names(virtual=False):
                ...
                if not vaex.array_types.same_type(type1, type2):
                    ...
                    def equal_mask(a, b):
                        ...
                        b = normalize(b)
                        boolean_mask = (a == b)
                        if not self.is_string(column_name) and self.dtype(column_name).kind == 'f':  # floats with nan won't equal itself, i.e. NaN != NaN
